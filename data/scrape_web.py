...
out_dir = (
    r"F:\Repos\expository-text-generation\data\datasets\processed"  # output directory
)
dataset_dir = r"F:\Repos\expository-text-generation\data\datasets\wiki_cs\content\wiki_cs"  # dataset directory
            ...
            for p in paragraphs_web:
                ...
                for sentence in curr_sentences:
                    if (
                        len(sentence) < 30
                        or "©" in sentence
                        or "license" in sentence.lower()
                        or "cookies" in sentence.lower()
...
columns = [
    "aspect",
    "title",
    "web_sentences_with_desc",
    "web_sentences_no_desc",
    "output",
    "output_aug",
]
...
for idx in tqdm.tqdm(range(num_train)):
    ...
    general_sentences_leak = shuffle_sentences(
        web_sentences + nltk.sent_tokenize(output)
    )
...
for idx in tqdm.tqdm(range(num_test)):
    ...
    general_sentences = shuffle_sentences(web_sentences)
    general_sentences_leak = shuffle_sentences(
        web_sentences + nltk.sent_tokenize(output_aug)
    )
...
new_train, new_test = (
    datasets.Dataset.from_dict(new_dataset_train),
    datasets.Dataset.from_dict(new_dataset_test),
)
